...
export function EditModal({ isOpen, onClose, onSave }: EditModalProps) {
  const { topicsList: topics } = useTopicTitle();
  ...
  const handleEditTopic = (oldTopic: string, newTopic: string) => {
    setLocalTopics((prevTopics) =>
      prevTopics.map((topic) => (topic === oldTopic ? newTopic : topic))
    );
  };

  const handleEditHashtag = (oldHashtag: string, newHashtag: string) => {
    setLocalHashtags((prevHashtags) =>
      prevHashtags.map((hashtag) =>
        hashtag === oldHashtag ? newHashtag : hashtag
      )
    );
  };
